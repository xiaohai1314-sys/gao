var rule = {
    ...
    headers: {
        ...
        'Cookie': getItem('cookie') || 'esc_search_captcha=1'
    },
    ...
    推荐: '.bt_img;ul&&li;*;*;*;*',
    double: true,
    一级: '.bt_img&&ul&&li;h3.dytit&&Text;img.lazy&&data-original;js:pdfh(html,\'.jidi&&Text\')||pdfh(html,\'.dy_remarks&&Text\')||\'\';a&&href',
    二级: {
        "title": "h1&&Text;.moviedteail_list li&&a&&Text",
        "img": "div.dyimg img&&src",
        "desc": ".moviedteail_list li:eq(3) a&&Text;js:pdfh(html,'.moviedteail_list li:eq(5) a&&Text')||pdfh(html,'.moviedteail_list li:eq(5)&&Text');.moviedteail_list li:eq(1) a&&Text;.moviedteail_list li:eq(7)&&Text;.moviedteail_list li:eq(5)&&Text",
    ...
    login: {
        ...
        action: `js:
            ...
            let html = request(loginUrl);
            let token = pdfh(html, 'input[name="__token__"]&&value');
            ...
            // 检查登录是否成功
            if (res.body.indexOf('登录成功') > -1 || res.body.indexOf('我的会员') > -1) {
                // 获取并保存Cookie
            ...
            } else if (res.body.indexOf('验证码') > -1) {
                // 处理需要验证码的情况
                let captchaUrl = pdfh(html, 'img.captcha&&src');
                if (captchaUrl) {
                    let captcha = captcha(captchaUrl);
                    data.verify = captcha;
                    
                    // 重新提交带验证码的请求
                    res = request(loginUrl, {
                        method: 'POST',
                        data: data,
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                    ...
                    if (res.body.indexOf('登录成功') > -1) {
                        ...
                        '验证码登录失败：' + pdfh(res.body, 'div.error&&Text');
                    }
                } else {
                    '需要验证码但未找到验证码图片';
                }
            } else {
                '登录失败：' + pdfh(res.body, 'div.error&&Text');
            }
        `
    },
    ...
    init: `js:
        // 检查Cookie是否有效
        let testUrl = HOST + '/user';
        let res = request(testUrl, {headers: rule.headers});
        if (res.body.indexOf('请登录') > -1 || res.body.indexOf('登录') > -1) {
            // Cookie失效，清除存储
            setItem('cookie', null);
            rule.headers.Cookie = 'esc_search_captcha=1';
            'Cookie已失效，请重新登录';
        } else {
            'Cookie有效';
        }
